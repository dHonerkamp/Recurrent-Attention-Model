...
class RetinaSensor(object):
    def __init__(self, FLAGS, name='RetinaSensor'):
        self.name = name
...
class GlimpseNetwork(object):
    def __init__(self, FLAGS, patch_shape, out_shape, name="GlimpseNetwork"):
        self.name = name
        with tf.variable_scope(self.name, reuse=False):
            self.flat_patch_shape = np.prod(patch_shape)
            self.size_hidden_g = 128
            self.size_hidden_l = 128
            self.size_hidden_gl2 = out_shape
...
class GlimpseNetwork_DRAM(object):
    def __init__(self, FLAGS, patch_shape, out_shape, name='GlimpseNetwork'):
        self.name = name
        # tf.AUTO_REUSE if using layer module as weights are only initialized upon first call
...
class LocationNetwork(object):
    def __init__(self, img_NHWC, FLAGS, name='LocationNetwork'):
        """
        :param img_NHWC: unused, for compatibility with locationNet_relative
        """
        self.name = name
        with tf.variable_scope(self.name, reuse=False):
            self.std = FLAGS.loc_std
            self.max_loc_rng = FLAGS.max_loc_rng
            self.loc_dims = [tf.shape(img_NHWC)[0], FLAGS.loc_dim]
    ...
    def initial_loc(self):
        with tf.variable_scope(self.name, reuse=True):
            loc = tf.random_uniform(self.loc_dims,
                                    minval=self.max_loc_rng * -1.,
                                    maxval=self.max_loc_rng *  1.)
            loc_mean = loc
        return loc, loc_mean


class Rewards(object):
    def __init__(self, FLAGS, name='Reward'):
        self.name = name
    ...
    def _rewards_clf(self, predictions, label):
    ...
    def _rewards_open_set(self, predictions, label):
    ...
    def __call__(self, predictions, label):
        with tf.variable_scope(self.name):
            if self.reward_type == "classification":
                rewards, reward, unknown_acc = self._rewards_clf(predictions, label)
            elif self.reward_type == "open_set":
                rewards, reward, unknown_acc = self._rewards_open_set(predictions, label)
